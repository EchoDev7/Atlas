...
class VPNUserResponse(BaseModel):
    ...
    @field_validator("enable_l2tp", mode="before")
    @classmethod
    def _coerce_optional_protocol_flags(cls, value):
        return bool(value) if value is not None else False

    @field_validator("enable_openconnect", mode="before")
    @classmethod
    def _coerce_enable_openconnect(cls, value):
        return True if value is None else bool(value)
...
class VPNConfigBase(BaseModel):
    ...
    protocol: str = Field(..., description="Protocol type (openvpn, wireguard, l2tp, singbox)")
